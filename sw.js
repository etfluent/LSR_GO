...
importScripts('https://www.lsr-go.com/lsr/wp-content/plugins/chrome-push-notifications/assets/js/library/Dexie.min.js');
...
self.addEventListener('push', function(event) {
  ...
  event.waitUntil(self.registration.pushManager.getSubscription().then(function(subscription) {
   if (_webPushConfig.debug) console.log('Push sub data', subscription);
      return fetch('https://www.lsr-go.com/lsr/?subId='+ encodeURIComponent(subscription.endpoint)).then(function(response) {  
...
function showNotification(title, body, image, tag) {
    var options = {
        body: body,
        tag: tag,
        icon: 'https://www.lsr-go.com/lsr/wp-content/uploads/2016/04/LSR-GOiconFin.png'
    };
